...
def test_documents():
    ...
    response = requests.post(f"{BASE_URL}/api/collections/{COLLECTION_NAME}/", json=doc_data)
    create_success = response.status_code == 201
    ...
    response = requests.get(f"{BASE_URL}/api/collections/{COLLECTION_NAME}/{doc_id}")
    get_success = response.status_code == 200
    ...
    response = requests.put(f"{BASE_URL}/api/collections/{COLLECTION_NAME}/{doc_id}", json=updated_doc_data)
    ...
    response = requests.get(f"{BASE_URL}/api/collections/{COLLECTION_NAME}/?limit=5&offset=0")
...
def test_bulk_operations():
    ...
    if new_doc_ids:
        response = requests.get(f"{BASE_URL}/api/collections/{COLLECTION_NAME}/{new_doc_ids[0]}")
...
def test_document_deletion():
    ...
    response = requests.delete(f"{BASE_URL}/api/collections/{COLLECTION_NAME}/{doc_id}")
    delete_success = response.status_code == 200
    
    # Verify document no longer exists
    verification_response = requests.get(f"{BASE_URL}/api/collections/{COLLECTION_NAME}/{doc_id}")
...
def test_collection_deletion():
    ...
    if DOCUMENT_IDS and delete_success:
        for doc_id in DOCUMENT_IDS:
            doc_response = requests.get(f"{BASE_URL}/api/collections/{COLLECTION_NAME}/{doc_id}")
